...
// Simplified Amazon biome boundary polygon (RAISG biogeographic boundary, simplified
// to ~50 points to stay within API limits while preserving the correct boundary).
const AMAZON_GEOJSON = {
...
async function registerGeostore() {
  const res = await fetch(`${BASE}/geostore`, {
    method: 'POST',
    headers: GFW_HEADERS,
    body: JSON.stringify({ geojson: AMAZON_GEOJSON })
  });
  const json = await res.json();
  if (!res.ok) throw new Error(`Geostore registration failed: ${JSON.stringify(json)}`);
  return json.data?.gfw_geostore_id || json.data?.id;
}

async function queryYear(version, geostoreId, year) {
  ...
  const url = `${BASE}/dataset/umd_tree_cover_loss/${version}/query/json?sql=${sql}&geostore_id=${geostoreId}`;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 2. Register Amazon boundary as geostore and get ID
    const geostoreId = await registerGeostore();
    if (!geostoreId) throw new Error('Could not register geostore');
    ...
    for (const year of years) {
      try {
        const ha = await queryYear(version, geostoreId, year);
    ...
    res.status(200).json({
      ...
      methodology: 'UMD primary forest loss · Amazon biome boundary · 30% canopy density',
      results,
