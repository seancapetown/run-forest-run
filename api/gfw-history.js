// /api/gfw-history.js
// One-time endpoint: registers Amazon biome boundary as GFW geostore,
// then queries umd_tree_cover_loss by year using that geostore_id.
// Geostore approach is required for raster datasets — inline geometry doesn't work.
...
async function registerGeostore() {
  const res = await fetch(`${BASE}/geostore`, {
    method: 'POST',
    headers: GFW_HEADERS,
    body: JSON.stringify({ geojson: AMAZON_FC })
  });
  const json = await res.json();
  if (!res.ok) throw new Error(`Geostore failed: ${JSON.stringify(json)}`);
  // GFW returns gfw_geostore_id
  const id = json.data?.gfw_geostore_id || json.data?.id || json.gfw_geostore_id;
  if (!id) throw new Error(`No geostore ID in response: ${JSON.stringify(json)}`);
  return id;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Register boundary and get geostore ID
    const geostoreId = await registerGeostore();

    // Single grouped query using geostore_id
    ...
    const apiRes = await fetch(
      `${BASE}/dataset/umd_tree_cover_loss/${version}/query/json?sql=${sql}&geostore_id=${geostoreId}`,
      { headers: GFW_HEADERS }
    );
    ...
    res.status(200).json({
      ...
      methodology: 'UMD primary forest loss · 30% canopy density · 9 Amazon countries',
