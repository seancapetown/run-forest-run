...
// Deploy to Vercel, hit the URL once in your browser, copy the numbers, then remove this file.
//
...
// This is the boundary GFW uses on their own Amazon dashboard.
...
async function queryYear(version, year) {
  ...
  const res = await fetch(url, { headers: { 'x-api-key': GFW_API_KEY } });
  const json = await res.json();
